...
import static nl.minvenj.pef.metal.CustomExpression.gtEqNum;
...
public final class IPv4 {
    ...
    public static final BinaryValueExpression HEADER_SIZE = mul(and(lastRef("versionihl"), con(0x0F)), con(4));

    /** Total packet size in octets. */
    public static final ValueExpression TOTAL_SIZE = lastRef("iplength");

    /** IPv4 header format definition. **/
    public static final Token HEADER = seq(
                                           def("versionihl", 1, and(eqNum(shr(self, con(4)), con(4)), gtEqNum(and(lastRef("versionihl"), con(0x0F)), con(5)))), // version + IHL; 0x4[5-F] = IPv4
    ...
    public static final Token KNOWN_PROTOCOL = new Token("knownprotocol", new Encoding()) {
        ...
        protected ParseResult parseImpl(final String scope, final Environment env, final Encoding enc) throws IOException {
            if (eqNum(lastRef("protocol"), con(Protocol.ICMP)).eval(env, enc)) {
                return ICMP.FORMAT.parse(scope, env, enc);
            }
            else if (eqNum(lastRef("protocol"), con(Protocol.UDP)).eval(env, enc)) {
                return UDP.FORMAT.parse(scope, env, enc);
            }
            else if (eqNum(lastRef("protocol"), con(Protocol.TCP)).eval(env, enc)) {
